...
- `waydroid` is a kernel shell builtin (`cmd_waydroid` in `kernel/kernel/waydroid.c`). Flows: `waydroid init` seeds the `android-stock` image (idempotent now — skipped if `/containers/images/android-stock/init` exists) and installs the 10 bundled apps into `<image>/system/app/<name>/<name>`; `session start|stop|pause|resume` drives the appvm container; `app list|launch <app>`; `shell` runs `/bin/sh` in the guest; `status` reports image/root/session/gui.
- Apps are userspace ELFs in `pkgs/core/<name>/src/`, built into `/bin/android-*` via `kernel/userspace/Makefile` (`ANDROID_PROGS` + `LIB_ANDROID` = `lib/android_ui.o`) and embedded by `gen_initramfs.py`; launched inside the container image via `container_exec(id, "/system/app/<name>/<name>")`.
