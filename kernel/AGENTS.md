...
- `appvm`/`container` shell commands work directly on the seeded image: `appvm images` enumerates `/containers/images/`, `appvm pull <debian-minimal|android-stock>` materializes an image, and `appvm run <image> <cmd...>` boots pid-1 then execs the command (exec requires the running state, so `run` starts first).
